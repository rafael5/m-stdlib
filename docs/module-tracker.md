...
| P3 | H2 | 31 | [`STDCOMPRESS`](modules/stdcompress.md) | `v0.3.x` (in flight on `main`) | ~3d ✅ scaffolded / ~2-4d remaining | T28 | `$ZF → libz`, `$ZF → libzstd`; A6 | gzip / gunzip / deflate / inflate / zstdCompress / zstdDecompress / available. Output via `.out` byref (16 MiB cap); errors via `$ECODE`. **Scaffolded 2026-05-07:** C shim (`src/callouts/stdcompress.c`), .xc descriptor (`tools/std_compress.xc`), M wrapper (`src/STDCOMPRESS.m`), test suite (28 labels in `tests/STDCOMPRESSTST.m`), per-module doc — all on `main`; lint clean (0E). **Remaining:** host needs `libzstd-dev` for the build; engine green-run blocked on T28 alongside STDCRYPTO. | 🟡 C13 — speculative P3 hookup; m-cli has no current compression need; queued behind T11 |
