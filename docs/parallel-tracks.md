...
| **L25** | v0.2.x (v0+T23+T24; v1 incremental) | STDXML | none (STDREGEX listed soft for future XPath) | ✅ **Landed on `main` 2026-05-07.** Eleventh Table 2 promotion — the long-deferred Pri 1 finally moves. v0+T23+T24 is ~50% of the 12-16d full envelope: well-formed XML 1.0 elements, attributes, nested children, text content, 5 standard entities + comments / PI / xml-decl / CDATA + numeric character references (UTF-8 encoded). Recursive-descent parser; tree shape mirrors STDJSON's caller-owned-tree convention; `childByName` does the internal `merge` to sidestep the `.x(SUBS)` YDB syntax limit. Public surface: `parse` / `valid` / `rootName` / `attr` / `text` / `childCount` / `childByName` / `lastError`. Remaining queued at T25-T27 (~7-11d): namespaces (T25), DTDs / DOCTYPE / custom entities (T26), XPath 1.0 query subset (T27). 75/75 assertions; 26/27 labels (96.3%); 0 lint errors. Module doc at `docs/modules/stdxml.md`. |
...
Phase 4   L15 STDCSPRNG                      ✅ landed on main 2026-05-07 — first Table 2 promotion (was Pri 1); /dev/urandom backend
          ...
          L25 STDXML v0+T23+T24              ✅ landed on main 2026-05-07 — eleventh Table 2 promotion; XML 1.0 + comments/PI/xml-decl/CDATA + numeric char refs (~50% of full envelope; T25-T27 queued)
