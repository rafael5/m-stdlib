...
| **L25** | v0.2.x (v0+T23+T24+T25-elt; v1 incremental) | STDXML | none (STDREGEX listed soft for future XPath) | ✅ **Landed on `main` 2026-05-07.** Eleventh Table 2 promotion. v0+T23+T24+T25(elt) is ~60% of the 12-16d full envelope: well-formed XML 1.0 + comments / PI / xml-decl / CDATA + numeric character references (UTF-8 encoded) + element-level namespaces. Recursive-descent parser; tree shape mirrors STDJSON's caller-owned-tree convention. Public surface: `parse` / `valid` / `rootName` / `attr` / `text` / `childCount` / `childByName` / `ns` / `lastError`. Tree shape: `node("name")` is the local name; `node("ns")` and `node("prefix")` track namespace info; `xmlns*` attributes are filtered out of `node("attr",...)`. Remaining queued at T25b/T26/T27 (~5-9d): attribute-namespace resolution (T25b), DTDs / DOCTYPE / custom entities (T26), XPath 1.0 query subset (T27). 105/105 assertions; 29/30 labels (96.7%); 0 lint errors. Module doc at `docs/modules/stdxml.md`. |
...
Phase 4   L15 STDCSPRNG                      ✅ landed on main 2026-05-07 — first Table 2 promotion (was Pri 1); /dev/urandom backend
          ...
          L25 STDXML v0+T23+T24+T25(elt)     ✅ landed on main 2026-05-07 — eleventh Table 2 promotion; XML 1.0 + comments/PI/xml-decl/CDATA + numeric char refs + element-level namespaces (~60% of full envelope; T25b/T26/T27 queued)
