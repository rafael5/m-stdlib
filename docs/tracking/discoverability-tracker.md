...
| WC1 | done | WC | 1 | vscode | VS Code extension v0.1: hover, goto-def, completion driven by `dist/stdlib-manifest.json` (no LSP) | WA4 | 2–3d | Extension activates on `.m` files; hover on `^STDJSON` and `parse^STDJSON` shows synopsis + signature; goto-def jumps to `src/STDJSON.m:L`; completion suggests `^STD*` modules and labels. | § 5.1 | new repo `~/projects/m-stdlib-vscode/` initial commit `1c15a05` (local; remote setup pending user creating GitHub repo) |
...
**Status.** done (2026-05-08; new repo `~/projects/m-stdlib-vscode/` initial commit `1c15a05`; not yet pushed — needs user to create the GitHub remote).
...
- **Push status**: not pushed. The `package.json` already points `repository.url` at `https://github.com/rafael5/m-stdlib-vscode`; the user creates the GitHub remote and pushes when ready (`git remote add origin ... && git push -u origin main`).
